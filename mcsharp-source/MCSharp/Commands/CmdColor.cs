...
namespace Overv
{
    public class CmdColor : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            {
                string color = Colour.Parse(message);
                if (color == "") { p.SendMessage("There is no color \"" + message + "\"."); }
        ...
        public override void Help(Player p)
        {
            ...
            p.SendMessage("Colors: &0black &1navy &2green &3teal &4maroon &5purple &6gold &7silver &8gray &9blue &alime &baqua &cred &dpink &Syellow &fwhite");
        }
        static string Name(string name)
        {
            string ch = name[name.Length - 1].ToString().ToLower();
            if (ch == "s" || ch == "x") { return name + "&S'"; }
            else { return name + "&S's"; }
        }
    }
}