/*
	Copyright 2010 MCSharp Team Licensed under the
	Educational Community License, Version 2.0 (the "License"); you may
	not use this file except in compliance with the License. You may
	obtain a copy of the License at
	
	http://www.osedu.org/licenses/ECL-2.0
	
	Unless required by applicable law or agreed to in writing,
	software distributed under the License is distributed on an "AS IS"
	BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
	or implied. See the License for the specific language governing
	permissions and limitations under the License.
*/
using System;

namespace Overv
{
    public class CmdWhois : Command
    {
        public override string name { get { return "whois"; } }
        public override string type { get { return "info"; } }
        public override LevelPermission defaultPerm { get { return LevelPermission.Guest; } }
        public CmdWhois() { }
        public override void Use( Player p, string message ) {
            Player who = null;
            if ( message == "" ) { who = p; } else { who = Player.Find( message ); }
            if ( who != null ) {
                if ( who == p ) {
                    message = "&SYou are " + who.group.color + who.group.name + "&S.";
                } else {
                    message = who.color + who.name + "&S is " +
                        who.group.color + who.group.name + "&S on &b" + who.level.name + "&S.";
                    message = who.color + who.name + "&S is on the " + who.team.color + who.team.name + "&S.";
                }

                if ( Server.afkset.Contains( who.name ) ) { message += "-AFK-"; }

                if ( p.group == Group.Find( "operator" ) || p.group == Group.Find( "superOp" ) ) {
                    message += " IP: " + who.ip + ".";
                }

                p.SendMessage( message );
            } else { p.SendMessage( "There is no player \"" + who + "\"!" ); }
        }

        public override void Help(Player p)
        {
            p.SendMessage("/whois [player] - Displays information about someone.");
        }
    }
}