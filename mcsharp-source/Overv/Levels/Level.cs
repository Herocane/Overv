...
namespace Overv {
    ...
    public class Level {
        ...
        public byte[] redRotation = { 0, 0 };
        public ushort[] blueSpawn = { 0, 0, 0 };
        public byte[] blueRotation = { 0, 0 };
                    ...
                    for ( x = 0; x < width; ++x ) {
                        for ( z = 0; z < height; ++z ) {
                            for ( y = 0; y < depth; ++y ) {
                                switch ( type ) {
                                    case "flat":
                                        SetBlock( x, y, z, ( ( y == half ) ? Block.grass : ( ( y > half ) ? Block.air : Block.dirt ) ) );
                                        break;
                                    case "pixel":
                                        if ( y == 0 ) {
                                            SetBlock( x, y, z, Block.blackrock );
                                        } else {
                                            if ( x == 0 || x == width - 1 || z == 0 || z == height - 1 ) {
                                                SetBlock( x, y, z, Block.white );
                                            }
                                        }
                                        break;
                                }
        ...
        public void Save() {
            ...
            try {
                ...
                Server.s.Log( "FAILED TO SAVE: " + name + " BECAUSE: " + e.ToString() );
        ...
        public void SaveProperties() {
            StreamWriter sw = new StreamWriter( File.Create( "levels/" + name + ".properties" ) );
            sw.WriteLine( "author = " + author );
            sw.WriteLine( "likes = " + likes );
            sw.WriteLine( "dislikes = " + dislikes );
            sw.WriteLine( "redSpawnX = " + redSpawn[0] );
            sw.WriteLine( "redSpawnY = " + redSpawn[1] );
            sw.WriteLine( "redSpawnZ = " + redSpawn[2] );
            sw.WriteLine( "redSpawnRotX = " + redRotation[0] );
            sw.WriteLine( "redSpawnRotY = " + redRotation[1] );
            sw.WriteLine( "blueSpawnX = " + blueSpawn[0] );
            sw.WriteLine( "blueSpawnY = " + blueSpawn[1] );
            sw.WriteLine( "blueSpawnZ = " + blueSpawn[2] );
            sw.WriteLine( "blueSpawnRotX = " + blueRotation[0] );
            sw.WriteLine( "blueSpawnRotY = " + blueRotation[1] );
            sw.WriteLine( "redFlagX = " + redFlag[0] );
            sw.WriteLine( "redFlagY = " + redFlag[1] );
            sw.WriteLine( "redFlagZ = " + redFlag[2] );
            sw.WriteLine( "blueFlagX = " + blueFlag[0] );
            sw.WriteLine( "blueFlagY = " + blueFlag[1] );
            sw.WriteLine( "blueFlagZ = " + blueFlag[2] );
            sw.WriteLine( "maxBuildHeight = " + maxBuildHeight );
            sw.WriteLine( "# divider is always along the X axis! Bear this in mind when making your maps." );
            sw.WriteLine( "divider = " + divider );
            sw.Flush();
            sw.Close();
            sw.Dispose();
        }
        ...
        public static Level Load( string name, byte phys ) {
            ...
            if ( File.Exists( path ) ) {
                ...
                try {
                    ...
                    Server.s.Log( "LOADED " + name + ".lvl..." );
                    return level;
                } catch ( Exception ex ) {
                    Server.s.Log( "ERROR loading level \"" + name + "\"." );
                    Server.s.Log( ex.StackTrace );
                    return null;
                } finally {
                    fs.Close();
                }
        ...
        public void LoadProperties() {
            if ( File.Exists( "levels/" + name + ".properties" ) ) {
                foreach ( string line in File.ReadAllLines( "levels/" + name + ".properties" ) ) {
                    if ( !line.StartsWith( "#" ) ) {
                        ...
                        switch ( key.ToLower() ) {
                            ...
                            case "redspawnx": redSpawn[0] = ushort.Parse( value ); break;
                            case "redspawny": redSpawn[1] = ushort.Parse( value ); break;
                            case "redspawnz": redSpawn[2] = ushort.Parse( value ); break;
                            case "redspawnrotx": redRotation[0] = byte.Parse( value ); break;
                            case "redspawnroty": redRotation[1] = byte.Parse( value ); break;
                            case "bluespawnx": blueSpawn[0] = ushort.Parse( value ); break;
                            case "bluespawny": blueSpawn[1] = ushort.Parse( value ); break;
                            case "bluespawnz": blueSpawn[2] = ushort.Parse( value ); break;
                            case "bluespawnrotx": blueRotation[0] = byte.Parse( value ); break;
                            case "bluespawnroty": blueRotation[1] = byte.Parse( value ); break;
                            case "redflagx": redFlag[0] = ushort.Parse( value ); break;
                            case "redflagy": redFlag[1] = ushort.Parse( value ); break;
                            case "redflagz": redFlag[2] = ushort.Parse( value ); break;
                            case "blueflagx": blueFlag[0] = ushort.Parse( value ); break;
                            case "blueflagy": blueFlag[1] = ushort.Parse( value ); break;
                            case "blueflagz": blueFlag[2] = ushort.Parse( value ); break;
                            case "maxbuildheight": maxBuildHeight = int.Parse( value ); break;
                            case "divider": divider = ushort.Parse( value ); break;
                        }
                    }
                }
            } else {
                ...
                if ( physics > 0 ) {
                    ...
                    if ( physics == 3 )       //Kakashis crazy code that I dont even know what it does.
                    {
                        ListCheck.ForEach( delegate( Check c ) {
                            ...
                            if ( PhysSand( c.b, 13 ) ) {
                                if ( GetBlock( (ushort)( x + 1 ), y, z ) != Block.Zero ) { PhysAir( PosToInt( (ushort)( x + 1 ), y, z ) ); }
                                if ( GetBlock( (ushort)( x - 1 ), y, z ) != Block.Zero ) { PhysAir( PosToInt( (ushort)( x - 1 ), y, z ) ); }
                                if ( GetBlock( x, y, (ushort)( z + 1 ) ) != Block.Zero ) { PhysAir( PosToInt( x, y, (ushort)( z + 1 ) ) ); }
                                if ( GetBlock( x, y, (ushort)( z - 1 ) ) != Block.Zero ) { PhysAir( PosToInt( x, y, (ushort)( z - 1 ) ) ); }
                                if ( GetBlock( x, (ushort)( y + 1 ), z ) != Block.Zero ) { PhysAir( PosToInt( x, (ushort)( y + 1 ), z ) ); }   //Check block above
                            }
                            c.time = 255;
                        } );
                    }
                        ...
                        try {
                            ...
                            switch ( blocks[C.b] ) {
                                ...
                                case Block.dirt:     //Dirt
                                    ...
                                    {
                                        if ( Block.LightPass( GetBlock( x, (ushort)( y + 1 ), z ) ) ) {
                                ...
                                case Block.water:         //Active_water
                                    ...
                                    if ( !PhysSpongeCheck( C.b ) ) {
                                        if ( GetBlock( x, (ushort)( y + 1 ), z ) != Block.Zero ) { PhysSandCheck( PosToInt( x, (ushort)( y + 1 ), z ) ); }
        ...
        private bool PhysSand( int b, byte type )   //also does gravel
        {
            ...
            if ( moved ) {
                AddUpdate( b, 0 );
                if ( physics == 2 ) { AddUpdate( tempb, type ); } else { AddUpdate( IntOffset( tempb, 0, 1, 0 ), type ); }
            }
