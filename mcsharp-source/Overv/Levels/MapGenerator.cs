...
namespace Overv
{
    ...
    public class MapGenerator
    {
                ...
                for ( int bb = 0; bb < terrain.Length; bb++)
                {
                    ...
                    if (z > WaterLevel)
                    {
                        ...
                        if (!type.Equals("ocean"))  //Oceans dont have trees
                        {
                            ...
                            {
                                if (Lvl.GetBlock(x, (ushort)(z + 1), y) == Block.air)    //No flowers here right?
                                {
                                    if (Lvl.GetBlock(x, z, y) == Block.grass)    //Im not on sand am I either?
                                    {
                                        if (rand.Next(13) == 0)
                                        {
        ...
        void AddTree(Level Lvl, ushort x, ushort z, ushort y, Random Rand)
        {
            ...
            {
                if (Lvl.GetBlock(x, (ushort)(z + zz), y) == Block.air)   //Not likly to trigger anyway
                {
            ...
            for (short xx = (short)-top; xx <= top; ++xx)
            {
                for (short yy = (short)-top; yy <= top; ++yy)
                {
                    ...
                    {
                        if (Lvl.GetBlock((ushort)(x + xx), (ushort)(z + zz + height), (ushort)(y + yy)) == Block.air)   //Not likly to trigger anyway
                        {
        ...
        private bool TreeCheck(Level Lvl, ushort x, ushort z, ushort y, short dist)         //return true if tree is near
        {
            for (short xx = (short)-dist; xx <= +dist; ++xx)
            {
                for (short yy = (short)-dist; yy <= +dist; ++yy)
                {
                    ...
                    {
                        if (Lvl.GetBlock((ushort)(x + xx), (ushort)(z + zz ), (ushort)(y + yy)) == Block.trunk)
                        {
                            return true;
                        }
                    }
